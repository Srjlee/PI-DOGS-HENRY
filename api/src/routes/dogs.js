...
const { validarPeso } = require("../utils/funciones.js");
...
router.get("/", async (req, res) => {
  ...
  if (!name) {
    ...
    try {
      const perros = await axios(URL_API);
      const filtrados = perros.data.filter((p) =>
...
router.get("/:id", async (req, res) => {
  ...
  if (id.length === 36) {
    try {
      ...
      if (!buscoPerro)
        return res.status(404).send(`No existe un perro con el "${id}"`);
    ...
    try {
      ...
      if (!buscado)
        return res.status(201).send("No esta tu amigo en la lista!");
      let resp = {
        id: buscado.id,
